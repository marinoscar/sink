...
- **Geo-blocked and private videos surface as 422.** The service scans stderr for keywords (`private`, `unavailable`, `geo`, `removed`, `not available`, `rate-limit reached`, `login required`) and maps matches to `UnprocessableEntityException`. All other non-zero exits from the prepare phase map to `502 Bad Gateway`.
- **Platform bot-gate surfaces as a distinct 422.** When yt-dlp stderr contains "Sign in to confirm", "rate-limit reached", "login required", or "Requested content is not available", the service rejects the request with a specific message naming the platform and directing the user to `docs/cookies.md`. The operator can resolve this by placing a Netscape-format cookies file for the relevant platform in `infra/compose/secrets/` (e.g. `instagram-cookies.txt` for Instagram, `youtube-cookies.txt` for YouTube). Each platform has its own independent file slot; a file present for one platform is never passed to yt-dlp for a request to a different platform. The files are mounted secrets: mounted read-only at `/run/secrets/` inside the container, git-ignored (only `.gitignore` and `README.md` are tracked in that directory), and never written to application logs (only the platform name and request hostname are logged at `LOG` level when cookies are used).
