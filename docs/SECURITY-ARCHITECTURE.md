...
- **YouTube bot-gate surfaces as a distinct 422.** When yt-dlp stderr contains "Sign in to confirm", the service rejects the request with a specific message directing the user to `docs/youtube-cookies.md`. The operator can resolve this by placing a Netscape-format `youtube-cookies.txt` in `infra/compose/secrets/`. This file is a mounted secret: it is mounted read-only at `/run/secrets/` inside the container, is git-ignored (only `.gitignore` and `README.md` are tracked in that directory), and is never written to application logs (only the request hostname is logged at `LOG` level when cookies are used). Cookie injection is restricted to YouTube hostnames; non-YouTube URLs never receive the `--cookies` flag even if the file is present.
