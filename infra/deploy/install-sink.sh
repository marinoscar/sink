#!/usr/bin/env bash
# =============================================================================
# install-sink.sh — Install Sink on VPS (end-to-end)
# =============================================================================
...
#   - DNS A record for sink.marin.cr pointing to this VPS
...
PROXY_CONF_DST="${PROXY_CONF_DIR}/sink.conf"
...
if docker exec proxy-nginx nginx -t 2>/dev/null; then
    ...
    log "  ERROR: Nginx config validation failed after installing sink.conf."
    log "  Check: docker exec proxy-nginx nginx -t"
    exit 1
fi
